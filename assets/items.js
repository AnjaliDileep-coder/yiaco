              ...
              <Grid style={{ marginTop: 15 }}>
                ...
                <Col>
                  <Button block onPress={this.addToWishlist.bind(this)} icon transparent style={{ backgroundColor: '#fdfdfd' }}>
                    <Icon style={{ color: "#00bfff" }} name='ios-heart' />
                  </Button>
                </Col>
              </Grid>
  ...
  renderImages({ item, index }) {
    return (
      <TouchableWithoutFeedback key={index} onPress={() => this.openGallery(index)}>
        <Image source={require('../assets/thermometer.jpg')} style={{ width: Dimensions.get('window').width, height: 300 }} resizeMode="cover" />
...
const dummyProduct = {
  id: 2,
  title: 'THERMOMETER',
  description: "Instrument for measuring temperature, often a sealed glass tube that contains a column of liquid, as mercury, that expands and contracts, or rises and falls, with temperature changes, the temperature being read where the top of the column coincides with a calibrated scale marked on the tube or its frame..",
  image: '',
  images: [
